    ...
    <div
        ...
        <form action="{{ route('user.changeUsername') }}" method="POST">
            ...
            <label for="username"
                ...
                Username')}}
                <input id="username" type="text" class="w-full rounded-xl font-normal p-2 @error('password')
                 border-vanilla @enderror" name="username" required autocomplete="current-password">
            </label>
            @error('username')
