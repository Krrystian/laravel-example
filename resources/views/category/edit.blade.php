...
<div class="w-full h-screen flex items-center justify-center">
    <div
        ...
        <h1 class="text-3xl text-center font-bold mb-4">Edit category</h1>
        <form action="{{ route('category.update') }}" method="POST">
            @csrf
            @method('PUT')
            <input type="hidden" name="id" value="">
            <label for="category"
                class="text-xl flex text-center text-nowrap gap-4 items-center font-bold mb-4">{{__('Category name')}}
                <input id="category" type="text" class="w-full rounded-xl font-normal p-2 border-vanilla resize-none"
                    name="category" required autocomplete="category" value="{{ $category->name}}">
                </input>
            </label>
